...
LLM_TEMPERATURE = 0.6           # Средняя температура для баланса креативности и структуры
LLM_MAX_TOKENS = 16000          # Увеличиваем лимит токенов
...
def create_custom_business_context() -> str:
    ...
    ## Формат кластеров
    1. Названия (name): понятные, на русском языке (например: "Проблемы с интернетом").
    2. ID (cluster_id): короткие, на английском, snake_case (например: "network_issues", "billing_questions").
    3. Описания: краткие и конкретные.
    """
